...
static void LCD_Configuration(void)
{
    ...
    kly_gpio_port_initialize(0); // 0 is port A.
    kly_gpio_port_initialize(1); // 1 is port B for D0~D15
    kly_gpio_port_initialize(2); // 2 is port C.

    /* PA.3(BL_PWM) */
    kly_gpio_port_config(0, GPIO_Pin_3, KLY_GPIO_CONFIG_OUTPUT_PUSH_PULL, KLY_GPIO_PULL_NONE);
    /* D0~D15 */
    kly_gpio_port_config(1, 0xFFFF, KLY_GPIO_CONFIG_OUTPUT_PUSH_PULL, KLY_GPIO_PULL_NONE);
    /* PC.0(RST), PC.1(WR), PC.2(RD) , PC.6(CS), PC.7(RS) */
    kly_gpio_port_config(2, GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_6 | GPIO_Pin_7,
                         KLY_GPIO_CONFIG_OUTPUT_PUSH_PULL, KLY_GPIO_PULL_NONE);

    RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;
    config = AFIO->MAPR;
    config &= ~(AFIO_MAPR_SWJ_CFG_Msk);
    config |= AFIO_MAPR_SWJ_CFG_JTAGDISABLE;
    AFIO->MAPR = config;
}
...
void LCD_Initializtion(void)
{
    ...
    kly_gpio_pin_clear(2, 0);		/* LCD_RESET */
    kly_delay_ms(100);
    kly_gpio_pin_set(2, 0);
    kly_gpio_pin_set(0, 3);  /* Enable back light */
    ...
    else if(DeviceCode == 0x8989)
    {
        ...
        kly_delay_ms(50);   /* Enable LCD Oscillator */
        ...
        kly_delay_ms(50);   /* 320*240 0x2B3F */
